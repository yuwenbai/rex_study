    ...
    public class DebugService
    {
        ...
        internal unsafe void DumpStack(StackObject* esp, RuntimeStack stack)
        {
            ...
            for (var i = start; i <= end; i++)
            {
                ...
                DLog.LOG(DLogType.Log, sb.ToString());
            }
            ...
            for (var i = stack.ValueTypeStackBase; i > stack.ValueTypeStackPointer;)
            {
                ...
                DLog.LOG(DLogType.Log, "----------------------------------------------");
                DLog.LOG(DLogType.Log, string.Format("{2}(0x{0:X8}){1}", (long)i, vt, leak ? "*" : ""));
            ...
            DLog.LOG(DLogType.Log, "Managed Objects:");
 
            for (int i = 0; i < mStack.Count; i++)
            {
                DLog.LOG(DLogType.Log, string.Format("({0}){1}", i, mStack[i]));
            }
#endif
        }
