...
namespace LitEngine
{
    ...
    public class ScriptTool
    {
        ...
        static public void ChangeShader(Transform _target, string _shadername)
        {
            if (_target == null)
            {
                DLog.LOG(DLogType.Error,"切换shader失败，target为空 ");
                return;
            }
            Shader tshader = Shader.Find(_shadername);
            if (tshader == null)
            {
                DLog.LOG(DLogType.Error,"切换shader失败，没找到 " + _shadername);
                return;
            }
            ...
            if (trander == null)
            {
                DLog.LOG(DLogType.Error,"切换shader失败，Rander为空 ");
                return;
            }
        ...
        static public void LoadSceneAsync(string _appname, string _scenename, System.Action _callback, LoadSceneMode _loadmode)
        {
            if (_callback == null)
            {
                DLog.LOG(DLogType.Error,"回调不可为空");
                return;
            }
