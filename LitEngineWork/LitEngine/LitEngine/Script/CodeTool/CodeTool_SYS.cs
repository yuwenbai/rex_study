...
namespace LitEngine
{
    ...
    {
        private Dictionary<string, Type> mAssembType = new Dictionary<string, Type>();
        private Dictionary<string, IType> mMapType = new Dictionary<string, IType>();
        ...
        override protected void DisposeNoGcCode()
        {
            mAssembType.Clear();
            mMapType.Clear();
            DLog.LOG(DLogType.Error, "Assembly 无法直接卸载.如有卸载需求请使用IL模式.");
        }
        #region Sys类型缓存
        public void AddAssemblyType(System.Reflection.Assembly _assembly)
        {
            mAssembly = _assembly;
            if (mAssembType == null)
                mAssembType = new Dictionary<string, Type>();
        ...
        override public IType GetLType(string _name)
        {
            ...
            DLog.LOG(DLogType.Error,"GetLType 没找到类型:" + _name + "|" + ttype);
            return null;
        }
        ...
        override public IType GetListChildType(IType _type)
        {
            ...
            if (_type.TypeForCLR.IsGenericType)
            {
                ...
                if (genericArgTypes != null && genericArgTypes.Length > 0)
                {
                    ...
                    if (ret == null)
                        DLog.LOG(DLogType.Error, genericArgTypes[0].Name);
                }
                else
                {
                    DLog.LOG(DLogType.Error,genericArgTypes);
                }
            }
            return ret;
        }
