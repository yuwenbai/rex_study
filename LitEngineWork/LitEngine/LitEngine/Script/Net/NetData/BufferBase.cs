...
namespace LitEngine
{
    namespace NetTool
    {
        public class BufferBase
        {
            ...
            {
                if (mIndex - mPos < SocketDataBase.mPacketTopLen) return false;
                short tlen = SReadShort(mBuffer, mPos);
                if (mIndex - mPos - SocketDataBase.mFirstLen < tlen) return false;
                return true;    
            }
            ...
            unsafe public static void GetNetValue(byte* pdata, byte[] _buffer, int _startindex, int _length)
            {
                ...
                {
                    DLog.LOG(DLogType.Error, "GetNetValue数组越界");
                    return;
                }
                if (IsHDate)
                {
            ...
            #region 写入工具
            unsafe public static void SetNetValue(byte* pdata, byte[] _buffer, int _startindex, int _length)
            {
                if (_startindex + _length -1 >= _buffer.Length)
                {
                    DLog.LOG(DLogType.Error, "SetNetValue数组越界");
                    return;
                }
                ...
                {
                    int iimax = _startindex + _length ;
                    for (int i = _startindex; i < iimax; i++)
                        _buffer[i] = *pdata++;
                }
            ...
            //不支持多个Data多线程同时处理
            private static byte mBuffer1 = 0;
            private static byte[] mBuffer2 = new byte[2];
            private static byte[] mBuffer4 = new byte[4];
            private static byte[] mBuffer8 = new byte[8];
            unsafe public static byte[] GetBuffer(int _src)
            {
                SetNetValue((byte*)&_src, mBuffer4, 0, sizeof(int));
                return mBuffer4;
            }
            unsafe public static byte[] GetBuffer(short _src)
            {
                SetNetValue((byte*)&_src, mBuffer2, 0, sizeof(short));
                return mBuffer2;
            }
            unsafe public static byte[] GetBuffer(long _src)
            {
                SetNetValue((byte*)&_src, mBuffer8, 0, sizeof(long));
                return mBuffer8;
            }

            unsafe public static byte[] GetBuffer(float _src)
            {
                SetNetValue((byte*)&_src, mBuffer4, 0, sizeof(float));
                return mBuffer4;
            }
