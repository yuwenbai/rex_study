    ...
    namespace NetTool
    {
        public class UDPNet : NetBase
        {
            ...
            virtual protected void CreatSendAndRecThread()
            {
                CreatSend();
                CreatRec();
                DLog.LOG(DLogType.Log, mNetTag + "建立连接完成");
            }
            ...
            virtual protected void SendMessageThread()
            {
                while (mStartThread)
                {
                    ...
                    {
                        DLog.LOG(DLogType.Error,mNetTag + e.ToString());
                    }
            ...
            virtual protected void ReceiveMessage()
            {
                while (mStartThread)
                {
                    ...
                    {
                        DLog.LOG(DLogType.Error, "UDP接收线程异常:" + e + "|" + "|" + mServerIP);
                    }
                    Thread.Sleep(1);
                }
            ...
            override protected void Processingdata(int _len, byte[] _buffer)
            {
                try
                {
                    ...
                    if (_len < SocketDataBase.mPacketTopLen) return;
                    ReceiveData tssdata = mReceiveDataList[mUseIndex];
                    mUseIndex++;
                    if (mUseIndex >= mReadyRecDataCount)
                        mUseIndex = 0;
                    tssdata.SetBuffer(_buffer, 0);
