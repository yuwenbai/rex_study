...
public class UIPopupList : UIWidgetContainer
{
	...
	public virtual void Show ()
	{
		if (enabled && NGUITools.GetActive(gameObject) && mChild == null && isValid && items.Count > 0)
		{
			...
			mBackground.width = BackgroundSize.x <= 0 ? Mathf.RoundToInt(x) : (int)BackgroundSize.x;
			mBackground.height = BackgroundSize.y <= 0 ? Mathf.RoundToInt(-y + bgPadding.y) : (int)BackgroundSize.y;
			...
			mHighlight.width = HighlightSize.x <= 0 ? Mathf.RoundToInt(w) : (int)HighlightSize.x; 
			mHighlight.height = HighlightSize.y <= 0 ?Mathf.RoundToInt(h) : (int)HighlightSize.y;
