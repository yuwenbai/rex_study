...
namespace UnityEditor.XCodeEditor
{
	...
	{
		Push,								//com.apple.Push
		InAppPurchase,						//com.apple.InAppPurchase
		iCloud,                             //com.apple.iCloud
		GameCenter,                         //com.apple.GameCenter
		Wallet,                             //com.apple.Wallet
		Siri,                               //com.apple.Siri
		PersonalVPN,						//com.apple.VPNLite
		NetworkExtensions,					//com.apple.NetworkExtensions
		InterAppAudio,						//com.apple.InterAppAudio
		ApplicationGroups,					//com.apple.ApplicationGroups.iOS
		DataProtection,						//com.apple.DataProtection 
		HomeKit,							//com.apple.HomeKit
		HealthKit,							//com.apple.HealthKit
		WAC,								//com.apple.WAC -(WAC:Wireless Accessory Configuration)
		ApplePay,							//com.apple.ApplePay
		Maps,								//com.apple.Maps.iOS
		AssociatedDomains,					//com.apple.SafariKeychain 
		KeychainSharing,					//com.apple.Keychain
		BackgroundModes,					//com.apple.BackgroundModes
	...
	public partial class XCProject : System.IDisposable
	{
		...
		public bool overwriteCapabilitySetting( string settingName, string newValue, string buildConfigName = "all") {
			Debug.Log("overwriteCapabilitySetting " + settingName + " " + newValue + " " + buildConfigName);
			foreach( KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations ) {
				//Debug.Log ("build config " + buildConfig);
				XCBuildConfiguration b = buildConfig.Value;
				if ( (string)b.data["name"] == buildConfigName || (string)buildConfigName == "all") {
					//Debug.Log ("found " + b.data["name"] + " config");
					buildConfig.Value.overwriteCapabilitySetting(settingName, newValue);
					modified = true;
				} else {
					//Debug.LogWarning ("skipping " + buildConfigName + " config " + (string)b.data["name"]);
				}
		...
		public bool overwriteStringSetting( string settingName, string newValue, string buildConfigName = "all") {
			Debug.Log("overwriteCapabilitySetting " + settingName + " " + newValue + " " + buildConfigName);
			foreach( KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations ) {
				//Debug.Log ("build config " + buildConfig);
				XCBuildConfiguration b = buildConfig.Value;
				if ( (string)b.data["name"] == buildConfigName || (string)buildConfigName == "all") {
					//Debug.Log ("found " + b.data["name"] + " config");
					buildConfig.Value.overwriteStringSetting(settingName, newValue);
					modified = true;
				} else {
					//Debug.LogWarning ("skipping " + buildConfigName + " config " + (string)b.data["name"]);
				}
		...
		public PBXDictionary AddFile( string filePath, PBXGroup parent = null, string tree = "SOURCE_ROOT", bool createBuildFiles = true, bool weak = false )
		{
			...
			if (filePath.EndsWith (".entitlements")) {
				Debug.LogError ("Skip file ended with .entitlements");
				return null;
			}
		...
		public bool AddFolder( string folderPath, PBXGroup parent = null, string[] exclude = null, bool recursive = true, bool createBuildFile = true )
		{
			...
			foreach( string directory in Directory.GetDirectories( folderPath ) ) {
				...
				if( directory.EndsWith( ".bundle" ) ) {
					...
					if (directory.EndsWith ("MagicWindow.bundle")) {
						string bundleFile = AutoBuild.IOSTools.FingerprintJSPath();
						Debug.LogWarning( "This is a special folder: " + directory );
						AddFile( bundleFile, newGroup,"SOURCE_ROOT", createBuildFile );
						continue;
					}
			...
			foreach( string file in Directory.GetFiles( folderPath ) ) {
				...
				if (file.EndsWith (".entitlements")) {
					Debug.Log("Adding .entitlements for Root");
					AddFile( file, _rootGroup, "SOURCE_ROOT", createBuildFile );
				}
		...
		public bool SettingProvisionStyle (bool isAutomatic) {
			string ProvisioningStyleValue;
			if (isAutomatic) {
				ProvisioningStyleValue = "Automatic";
			} else {
				ProvisioningStyleValue = "Manual";
			}

			if (_project.FirstTargetAttribute.ContainsKey ("ProvisioningStyle")) {
				_project.FirstTargetAttribute ["ProvisioningStyle"] = ProvisioningStyleValue;
				Debug.Log( _project.FirstTargetAttribute );
				Debug.Log( ProvisioningStyleValue+ " replace" );
			} else {
				_project.FirstTargetAttribute.Add ("ProvisioningStyle", ProvisioningStyleValue);
				Debug.Log( ProvisioningStyleValue+ " add" );
				Debug.Log( _project.FirstTargetAttribute );
			}
		...
		public string CapabilityKey (Capability capability) {

			switch (capability) {
			case Capability.Push:
				return "com.apple.Push";
			case Capability.InAppPurchase: 
				return "com.apple.InAppPurchase";
			case Capability.iCloud: 
				return "com.apple.iCloud";
			case Capability.GameCenter: 
				return "com.apple.GameCenter";
			case Capability.Wallet: 
				return "com.apple.Wallet";
			case Capability.Siri: 
				return "com.apple.Siri";
			case Capability.PersonalVPN: 
				return "com.apple.VPNLite";
			case Capability.NetworkExtensions: 
				return "com.apple.NetworkExtensions";
			case Capability.ApplicationGroups: 
				return "com.apple.ApplicationGroups.iOS";
			case Capability.InterAppAudio: 
				return "com.apple.InterAppAudio";
			case Capability.DataProtection: 
				return "com.apple.DataProtection";
			case Capability.HomeKit: 
				return "com.apple.HomeKit";
			case Capability.HealthKit: 
				return "com.apple.HealthKit";
			case Capability.ApplePay: 
				return "com.apple.ApplePay";
			case Capability.Maps: 
				return "com.apple.Maps.iOS";
			case Capability.AssociatedDomains: 
				return "com.apple.SafariKeychain";
			case Capability.KeychainSharing: 
				return "com.apple.Keychain";
			case Capability.BackgroundModes: 
				return "com.apple.BackgroundModes";
			}

			return "invalid";
		}

		public bool AddCapability(Capability capability,bool isAvalible = true) {
			PBXDictionary capabilityChild = new PBXDictionary();
			capabilityChild.Add ("enabled", isAvalible);
			return AddCapability (CapabilityKey(capability),capabilityChild);
		}

		public bool AddCapability (string capability, PBXDictionary contentDic) {

			Debug.Log( capability );

			if (capability == "invalid") {
				Debug.Log ("capabilityKey is invalid");
				return false;
			}

			if (contentDic == null || capability == null) {
				Debug.Log( capability+ " don`t add" );
				return false;
			}

			if (_project.SystemCapabilities.ContainsKey (capability)) {
				_project.SystemCapabilities [capability] = contentDic;
				Debug.Log( _project.SystemCapabilities );
				Debug.Log( capability+ " replace" );
			} else {
				_project.SystemCapabilities.Add (capability, contentDic);
				Debug.Log( capability+ " add" );
				Debug.Log( _project.SystemCapabilities );
			}
		...
		{	
			...
			foreach( string headerpath in mod.headerpaths ) {
				...
				}else if (headerpath.Contains("$(SRCROOT)")){
					Debug.Log ("not prepending a path to " + headerpath);
					this.AddHeaderSearchPaths( headerpath );
				} else {
			...
			foreach( string libraryPath in mod.libraryPaths ) {
				if (libraryPath.Contains("$(inherited)")) {
					Debug.Log ("not prepending a path to " + libraryPath);
					this.AddLibrarySearchPaths( libraryPath );
				}else if (libraryPath.Contains("$(SRCROOT)")){
					Debug.Log ("not prepending a path to " + libraryPath);
					this.AddLibrarySearchPaths( libraryPath );
				}else {
					string absoluteHeaderPath = System.IO.Path.Combine( mod.path, libraryPath );
