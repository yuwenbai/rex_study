...
public class SQLiteDemo
{
    ...
    /// 创建一张表，填充数据
    /// </summary>
    public static void CreatTabel()
    {
        //创建名为sqlite4unity的数据库
        SQLiteHelper sql = new SQLiteHelper("data source=sqlite4unity.db");
        ...
        //创建名为table2的数据表
        sql.CreateTable("table2", new string[] { "ID", "Name", "Num", "Doc" }, new string[] { "TEXT", "TEXT", "TEXT", "TEXT" });

        //插入两条数据
        sql.InsertValues("table2", new string[] { "'10012'", "'你是谁'", "'112'", "'这是1段描述'" });
        sql.InsertValues("table2", new string[] { "'200125'", "'他是谁'", "'1545'", "'这是2段描述'" });
        sql.InsertValues("table2", new string[] { "'311452'", "'我是谁'", "'13245'", "'这是3段描述'" });
