    ...
    void OnGUI()
    {
        if (GUILayout.Button("LoadAssetbundle"))
        {
            ...
            if (manifestBundle != null)
            {
                ...
                //加载我们需要的文件;"
                AssetBundle cubeBundle = AssetBundle.LoadFromFile(Application.streamingAssetsPath + "/Android/model/prefab_hand/hand_man.data");
                Debug.Log(cubeBundle == null);
                GameObject cube = cubeBundle.LoadAsset("Hand_Man.prefab") as GameObject;
                Debug.Log(cube == null);
                if (cube != null)
                {
                    Instantiate(cube);
